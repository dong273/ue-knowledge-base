...
**Checklist**:
1. Module is listed in `.uproject` or `.upluginModules` array
...
| `UndefinedIdentifierWarningLevel` mismatch | Editor Target uses old BuildSettingsVersion — upgrade to V6 |

---

## Editor Target: UndefinedIdentifierWarningLevel Mismatch

**Error message pattern:**
`
ProjectNameEditor modifies the values of properties: [ UndefinedIdentifierWarningLevel: Off != Error ].
This is not allowed, as ProjectNameEditor has build products in common with UnrealEditor.
Remove the modified setting, change ProjectNameEditor to use a unique build
environment by setting 'BuildEnvironment = TargetBuildEnvironment.Unique;' in the
ProjectNameEditorTarget constructor, or set bOverrideBuildEnvironment = true to
force this setting on.
`

**Cause:** The Editor `.Target.cs` uses `DefaultBuildSettings = BuildSettingsVersion.V5` (or older), while UE 5.7 defaults to `V6`. When compiled with a newer MSVC toolchain (VS 2025/2026, MSVC 14.40+), the compiler flags differ between what the project specifies and what the engine specifies, and UBT detects the mismatch on shared build products.

**Fix (recommended):** Upgrade both Target.cs files to V6, and add `bOverrideBuildEnvironment = true` to the Editor target:

`csharp
// Both .Target.cs files
DefaultBuildSettings = BuildSettingsVersion.V6;

// Editor .Target.cs only
bOverrideBuildEnvironment = true;
`

**Alternative:** Set `BuildEnvironment = TargetBuildEnvironment.Unique;` — but this prevents Editor target cache sharing with the engine.

**Also relevant:** When upgrading to V6, also update `IncludeOrderVersion` to match:
`csharp
