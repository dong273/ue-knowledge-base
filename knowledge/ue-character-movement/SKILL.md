...
### Ground Penetration / Jump-Through-Ground

Character jumping through the ground is a common bug from several compounding factors. **First debugging step**: enable `GetCharacterMovement()->bShowDebug = true;` to observe MovementMode transitions in real-time.

**Causes and fixes (prioritized):**

| # | Cause | Fix |
|---|-------|-----|
| 1 | **No physics substepping** (Chaos UE5) | Set `MaxSubsteps=4`, `MaxSubstepDeltaTime=0.008` in Project Settings → Physics |
| 2 | **Ground mesh too thin** (single-face Plane) | Thicken collision hull to at least capsule radius/2 (10-20cm) |
| 3 | **JumpZVelocity too high** (>1500) | Lower velocity, or increase `MaxSimulationIterations` to 16 |
| 4 | **WalkableFloorAngle too tight** | Default 44.765°. If ground slants slightly, `IsWalkableFloor()` returns false |
| 5 | **Wrong collision profile** | Capsule must `Block` the ground's channel. Check with `ShowDebug COLLISION` |
| 6 | **Complex collision used as simple** | Ground mesh set to `UseComplexAsSimple` but `bTraceComplex=false` |
...
**WASD mapped to single Vector2D InputAction (all keys move same direction):**
Keyboard keys produce FVector(1,0,0) when pressed. When mapped to an Axis2D action without modifiers, all keys output the same direction. SwizzleAxis/Negate modifier chains cannot reliably separate axes because `EInputAxisSwizzle` has no identity operation (only YXZ, ZYX, XZY, YZX, ZXY). **Fix**: use 4 separate Boolean InputActions (IA_MoveForward/Backward/Right/Left), each bound to its own callback calling `AddMovementInput(Dir, ±1.f)`. See `references/third-person-setup.md`.

---

## Reference Files

- `references/top-down-hades-setup.md` — Complete Hades-style top-down character setup: fixed overhead camera, mouse-based aiming (`FaceMouseCursor`), `GetHitResultUnderCursor`, WASD absolute world-space movement (not camera-relative), 5-input-action ability slots (Attack/Special/Cast/Call/Dash), dash via `LaunchCharacter` + `MOVE_Flying`
...
- `references/third-person-setup.md` — Complete third-person character setup: CameraBoom + FollowCamera, inline Enhanced Input (no asset files), CMC config, sprint toggle, and UnrealMCP caveats for creating Blueprint children from C++ classes
