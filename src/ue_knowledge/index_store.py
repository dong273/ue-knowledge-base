...
def _legacy_content(root: Path) -> bool:
    if not root.exists():
        return False
    ignored = {CURRENT_FILE, GENERATIONS_DIR}
...
def load_current(root: Path) -> Path:
    ...
    if not generation.is_dir() or (generation / "BUILDING").exists():
...
def cleanup_generations(root: Path, keep: int = 2) -> None:
    ...
    complete = [
        ...
        if path.is_dir() and not (path / "BUILDING").exists()
    ]
...
def discard_incomplete(generation: Path) -> None:
    try:
