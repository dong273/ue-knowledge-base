...
**WASD -> Axis2D**: ⚠️ Do NOT use a single Axis2D action with modifier chains for WASD. Keyboard keys produce `FVector(1,0,0)` when pressed (same raw value for all keys). `SwizzleAxis`/`Negate` cannot reliably separate axes because `EInputAxisSwizzle` has no identity operation. The D key (no modifier) outputs `Vector2D(1,0)` but S also outputs `Vector2D(1,0)` not `Vector2D(0,-1)`.

**Correct pattern**: Use 4 separate `Boolean` InputActions, one per direction. Each callback calls `AddMovementInput(Dir, ±1.f)` directly with camera-relative rotation. See `ue-character-movement → references/third-person-setup.md` for the complete pattern.

**Gamepad stick**: `DeadZone(Radial, LowerThreshold=0.2)` per stick mapping. Sticks produce proper FVector2D axis separation, so modifiers work as expected.
...
- **WASD on single Axis2D action (all keys same direction)**: Keyboard keys produce `FVector(1,0,0)`. Modifier chains cannot separate them. Use 4 separate Boolean InputActions per direction. See `ue-character-movement → references/third-person-setup.md`.
