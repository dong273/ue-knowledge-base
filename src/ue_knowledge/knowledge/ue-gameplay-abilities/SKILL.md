...
description: Covers GAS, Gameplay Ability System, GameplayAbility, GameplayEffect, AttributeSet, GameplayTags, ability system, buffs, debuffs, cooldowns, or attribute modification. Also when integrating GAS with Enhanced Input — Sprint/Jump ability binding, attribute-driven speed changes, layer separation architecture. See references/ for detailed setup patterns, effect configuration, and gas-input-integration.md for Enhanced Input patterns.
---
...
## UE 5.7 API Migration (from UE 5.3-5.5)

The following GAS API changes in UE 5.7 broke code written for earlier UE5 versions. Fix all occurrences when porting.
...
// Fix: use ASC->GetNumericAttribute() directly
...
{
    // ❌ Compiler error C2065: 'GrantedTags': undeclared identifier
    // GrantedTags.AddTag(FGameplayTag::RequestGameplayTag(TEXT("State.Exhausted")));
}

// Fix: set DynamicGrantedTags on the FGameplayEffectSpec at application time
void ApplyExhaustion(UAbilitySystemComponent* ASC)
{
...
// Fix: mark handler as UFUNCTION() and use AddDynamic
// In header:
//    UFUNCTION()
...
// UE 5.7 — RENAMED to CancelAbilities, takes const pointer and optional WithoutTags
...
**Additional pitfalls:**
- The method signature is `CancelAbilities(const FGameplayTagContainer* WithTags, const FGameplayTagContainer* WithoutTags=nullptr, UGameplayAbility* Ignore=nullptr)` — all parameters are **pointers**, not references. Passing a stack-local container by address with `&var` is correct.
- Use `FGameplayTagContainer` constructor that takes a single tag for one-liners: `const FGameplayTagContainer CancelTags(FGameplayTag::RequestGameplayTag("Ability.Player.Sprint"));`
- `CancelAbilities` does NOT automatically cancel abilities whose `ActivationOwnedTags` match. It cancels abilities whose **AbilityTags** (the tags defined in the ability CDO constructor via `SetAssetTags`) contain any tag in `WithTags`. If you set tags via `SetAssetTags`, make sure the tag used for cancellation matches exactly.
...
- `references/ue5.7-api-migration.md` — UE 5.7 GAS API changes and fixes (ActivateAbility sig,
  GetFloatAttributeFromAbilitySystem removal, GrantedTags migration, dynamic delegate binding)
- `references/gas-input-integration.md` — Layer separation architecture (GAS + Enhanced Input):
  movement layer (WASD direct), ability layer (Sprint/Jump via GAS), bridge layer (attribute →
  MaxWalkSpeed). Covers Sprint/Jump implementation patterns, attribute bridge, acceleration/slow
  buffs via MovementSpeed attribute, stun blocking via tags.
